...
export class Model {
  ...
  getPreviewInfo(URL) {
    return window.fetch(`https://api.linkpreview.net/?key=${this.APIKEY}&q=${URL}`)
  ...
  checkURLList(newURL) {
    let URLmatch = this.URLCardsList.find(URLCard => {
      if (URLCard === newURL) return URLCard});
    return URLmatch;
  }
  ...
  deleteThisCard(deletedCard) {
    this.URLCardsList = this.URLCardsList.filter(URLCard => deletedCard !== URLCard);
    storage.toLocalSet(this.URLCardsList);
    return this.URLCardsList;
  }
}
